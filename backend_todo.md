...
- [ ] **⚪ DEFERRED — Revert COOP header to `same-origin`** — Now that Google Sign-In uses the redirect flow (authorization code) instead of popup, `SECURE_CROSS_ORIGIN_OPENER_POLICY = 'same-origin-allow-popups'` in `settings.py` is no longer needed. Revert to Django's default `'same-origin'` for stricter security. Low priority — `same-origin-allow-popups` is still safe, just slightly more permissive than necessary.
...
- [ ] **🔴 IMMEDIATE — One-click tailored resume from crawled job** — `POST /api/v1/jobs/<job_id>/tailor-resume/` accepts `job_id` + optional `resume_id` (defaults to user's default resume). Pipeline: scrape full JD from job URL → analyze resume vs JD → generate tailored resume with strict guardrails. **Guardrails:** (1) Rewrite ONLY summary + role responsibilities/bullets. (2) Add only skills learnable in <24h (communication, soft skills, basic tools). (3) NEVER fabricate certifications, degrees, experience, or projects. (4) Preserve all factual data verbatim (contact, education, dates, company names). (5) Target ATS score 80%+. **Model changes:** `DiscoveredJob.full_description`, `GeneratedResume.source_job` FK, `GeneratedResume.verification_score`. **Cost:** 2 credits (`resume_tailor` slug). **Post-generation:** lightweight re-analysis to verify ATS score ≥80%.
...
- [ ] **🔵 P3 — Make the test suite pass on Postgres (fix pgvector test-DB setup)** — CI now provisions Postgres+pgvector and validates that migrations *apply* on it, but the **test suite still runs on SQLite**. Running it on Postgres (`TEST_ON_POSTGRES=True`, added to settings) surfaces ~72 errors: `column "embedding" of relation "analyzer_discoveredjob" does not exist` during test-DB creation. A normal `manage.py migrate` on Postgres DOES create the column, but the test-runner's DB setup does not — the `embedding` field is model-defined only on Postgres (`_HAS_PGVECTOR`) while the column is added via raw SQL in `0014` + state-only in `0016`, and that split doesn't reproduce during test-DB build.
  **Deferred because:** this is the same conditional-pgvector-field debt noted above; fixing it means restructuring how the vector column is created (declare the field unconditionally with `SeparateDatabaseAndState`, or add the column in a normal schema operation guarded for non-Postgres) and re-testing the whole vector path. It's real, valuable work (it would unlock actual testing of job-matching/embedding code), but it's a dedicated migration change, not part of "add CI." Until then, `manage.py test` runs on SQLite, which is green.
