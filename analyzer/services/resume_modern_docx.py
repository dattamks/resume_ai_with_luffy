"""
Modern resume DOCX renderer.

Clean contemporary layout with teal accents and sans-serif typography.
...
COLOR_PRIMARY = RGBColor(0x0d, 0x73, 0x77)      # Teal
COLOR_SECONDARY = RGBColor(0x14, 0x91, 0x9b)    # Lighter teal
COLOR_BODY = RGBColor(0x32, 0x32, 0x32)          # Dark grey
COLOR_MUTED = RGBColor(0x6b, 0x72, 0x80)         # Muted grey
COLOR_DIVIDER = RGBColor(0x99, 0xd5, 0xc9)       # Light teal
...
def _build_contact(doc, content):
    ...
    # Name with teal background (simulated via shading)
    p = doc.add_paragraph()
...
def render_modern_docx(resume_content: dict) -> bytes:
    """Render structured resume JSON into a modern-styled DOCX."""
    doc = Document()
