...
flower: test -n "${FLOWER_PASSWORD}" || { echo "FLOWER_PASSWORD must be set"; exit 1; }; celery -A resume_ai flower --port=${PORT:-5555} --basic-auth=${FLOWER_USER:-admin}:${FLOWER_PASSWORD} --broker_api= --persistent=True --db=/tmp/flower.db
